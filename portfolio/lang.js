...
(function () {
    var translations = {
        ko: {
            ...
            'exp.cj.desc': 'LangChain 기반 멀티에이전트 파이프라인으로 한국 드라마 대본 분석 시스템을 개발했어요.',
            'exp.embedded.desc': '확산 모델의 추론 단계를 더 효율적으로 만드는 최적화 기법을 연구했어요.',
            'exp.troja.desc': '초기 스타트업에서 딥페이크 방어 백엔드와 MVP 웹페이지 구축을 담당했어요.',
            ...
            // Awards
            'award.awards': '수상',
            'award.awards.desc': 'UNIST에 수석 입학하여 Unistar 장학생으로 선발되었어요.',
            'award.teaching': '교육',
            'award.teaching.desc': 'AI 개론과 프로그래밍 1 과목에서 조교로 활동했어요.',
            ...
            // Korean Medical LLM
            'medllm.overview': '한국어 의료 데이터 부족 문제를 풀기 위해 약 44만 9,500건 규모의 도메인 데이터셋을 구축하고, `google/gemma2-9b` 기반 SFT와 모델 병합을 적용해 한국어 의료 특화 LLM을 개발했어요.',
            'medllm.problem': '문제: 범용 LLM은 의료 질문에서 환각이 발생하기 쉽고, 한국어 의학 용어 이해도도 충분하지 않아 신뢰가 중요한 의료 환경에 바로 쓰기 어려워요.',
            'medllm.goal1': '목표 1: 국가고시, 질병사전, 의료 법률 문서처럼 신뢰할 수 있는 출처를 바탕으로 한국어 의료 데이터셋을 구축해요.',
            'medllm.goal2': '목표 2: SFT와 모델 병합을 통해 의료 전문성을 높이면서도 일반적인 추론 능력은 최대한 유지해요.',
            'medllm.goal3': '목표 3: 한국어 의료 질의응답에 실제로 활용할 수 있는 특화 LLM 파이프라인을 완성해요.',
            'medllm.feat1': '하이브리드 데이터셋 설계: KorMedMCQA, 아산병원 질병사전, 번역한 MedExpQA/UltraMed, 의료 법률 기반 합성 QA를 포함해 449,500건의 학습 데이터를 구성했어요.',
            'medllm.feat2': '최신성 보강: 2025년도 의사국가고시 문항을 새로 라벨링해 데이터의 시의성을 높였어요.',
            'medllm.feat3': '데이터 중심 접근: 의료 전문가의 직접 라벨링이 어려운 상황에서, 공신력 있는 기관 데이터와 고품질 번역 데이터를 조합해 도메인 지식을 보강했어요.',
            'medllm.feat4': 'Instruction Tuning 최적화: 단순 텍스트가 아니라 QA/대화 형식 중심으로 데이터를 설계해 지시 이행 능력을 높였어요.',
            'medllm.chal1': 'Catastrophic Forgetting: 의료 데이터 비중이 지나치게 높아지면 일반 상식과 범용 추론 성능이 눈에 띄게 떨어졌어요.',
            'medllm.chal2': '혼합 비율 탐색: 선행연구와 반복 실험을 바탕으로 한국어:영어 60:40 비율이 가장 안정적이라는 결론을 얻었어요.',
            'medllm.chal3': '도메인-범용 균형: General:Medical 약 2.5:1 비율을 적용해 범용성과 전문성의 균형점을 맞췄어요.',
            'medllm.chal4': '모델 병합 전략: SFT 모델과 범용 모델을 병합해 의료 성능은 유지하면서 일반 대화 품질 하락을 줄였어요.',
            'medllm.result1': '성과: KorMedMCQA와 자체 구축 데이터를 바탕으로 한국어 의료 LLM 학습 파이프라인을 완성했어요.',
            'medllm.result2': '성과: 데이터 혼합 비율과 모델 병합을 통해 도메인 성능과 범용성 사이의 트레이드오프를 효과적으로 조정했어요.',
            'medllm.result3': '인사이트: 단순히 의료 데이터를 많이 넣는 것보다, 어떤 비율로 섞느냐가 성능에 더 큰 영향을 준다는 점을 확인했어요.',
            'medllm.result4': '인사이트: 저자원 언어 의료 LLM에서는 고품질 번역 데이터와 합성 데이터 활용 전략이 특히 중요하다는 점을 확인했어요.',

            // Deepfake Defense
            'dfdefense.overview': '딥페이크 생성 모델과 방어 모델을 함께 서빙하기 위해 NAVER Cloud Platform 위에 안정적인 백엔드 서버를 구축했어요. 이 시스템은 `inswapper` 기반 생성 모델과 LEAT, DIPA 같은 방어 기법을 한 환경에서 검증할 수 있게 해줘요.',
            'dfdefense.problem': '문제: 딥페이크는 개인 프라이버시와 정보 무결성을 크게 위협하지만, 많은 방어 기법은 연구 환경 안에서만 검증되어 실제 배포 관점의 테스트가 부족해요.',
            'dfdefense.goal1': '목표 1: 생성 모델과 방어 모델을 모두 서비스할 수 있는 프로덕션 수준의 백엔드를 구축해요.',
            'dfdefense.goal2': '목표 2: 무단 얼굴 스왑 공격으로부터 이미지를 보호하기 위해 LEAT 및 DIPA 방어 메커니즘을 구현해요.',
            'dfdefense.goal3': '목표 3: 확장 가능하고 접근성 있는 방어 전략 테스트 환경을 클라우드 위에 배포해요.',
            'dfdefense.feat1': '딥페이크 생성 파이프라인: 방어 기법을 체계적으로 테스트하기 위해 `inswapper` 모델을 통합했어요.',
            'dfdefense.feat2': 'LEAT 적용: 얼굴 스왑을 방해하는 미세한 섭동을 추가해 이미지를 보호하는 적대적 방어 기법을 배포했어요.',
            'dfdefense.feat3': 'DIPA 통합: 시각적 품질은 유지하면서 딥페이크 생성을 방해하는 이미지 섭동 기법을 구현했어요.',
            'dfdefense.feat4': '클라우드 아키텍처: NCP 위에서 Flask, uWSGI, Nginx를 조합해 확장 가능한 백엔드를 설계했어요.',
            'dfdefense.feat5': 'API 설계: 이미지 업로드, 처리, 방어 적용을 위한 RESTful 엔드포인트를 구축했어요.',
            'dfdefense.chal1': 'GPU 메모리 관리: 여러 딥러닝 모델을 동시에 운영해야 해서 GPU 메모리를 효율적으로 배분할 필요가 있었어요. 이를 위해 모델 로딩/언로딩 전략과 배치 처리를 도입했어요.',
            'dfdefense.chal2': '실시간 처리: 방어 기법이 빠르게 이미지를 처리해야 했기 때문에 PyTorch 최적화와 CUDA 가속으로 허용 가능한 지연 시간을 맞췄어요.',
            'dfdefense.chal3': '모델 간 형식 차이: 방어 방식마다 입출력 형식이 달라 통합이 어려웠고, 이를 해결하기 위해 공통 전처리·후처리 파이프라인을 만들었어요.',
            'dfdefense.chal4': '프로덕션 안정성: uWSGI 워커 관리, Nginx 로드밸런싱, 오류 처리를 정리해 서비스 안정성을 확보했어요.',
            'dfdefense.result1': '성과: 생성 모델과 방어 모델을 모두 서빙하는 안정적인 프로덕션 환경을 구축했어요.',
            'dfdefense.result2': '성과: LEAT와 DIPA는 95% 이상의 SSIM을 유지하면서 얼굴 스왑 시도를 효과적으로 방해했어요.',
            'dfdefense.result3': '배운 점: 적대적 ML, 섭동 기반 방어, 그리고 딥러닝 모델의 실제 배포 과정에 대한 이해가 깊어졌어요.',
            'dfdefense.result4': '배운 점: 클라우드 배포, 컨테이너화, 확장 가능한 ML 서빙 인프라 설계 역량을 쌓았어요.',

            // Bilingual Translation LLM
            'bilingual.overview': '제주 방언과 표준 한국어를 양방향으로 번역하기 위해 KoT5/KoBART 기반 인코더-디코더 모델을 구축하고, PEFT(LoRA, QLoRA)를 적용해 저자원 환경에서도 성능을 확보한 프로젝트예요.',
            'bilingual.problem': '문제: 번역 과제에서는 인코더-디코더 구조가 여전히 강점이 있지만, 제한된 GPU 환경에서 고품질 방언 번역 모델을 학습하면서 뉘앙스까지 살리기는 쉽지 않아요.',
            'bilingual.goal1': '목표 1: 단일 모델로 제주 방언↔표준어 양방향 번역이 가능한 통합 파이프라인을 구축해요.',
            'bilingual.goal2': '목표 2: Full Fine-tuning, LoRA, QLoRA를 비교해 성능과 자원 효율의 균형점을 찾아요.',
            'bilingual.goal3': '목표 3: 제한된 리소스에서도 실무 적용 가능한 정량 성능을 확보해요.',
            'bilingual.feat1': '데이터셋 엔지니어링: AIHub 제주 방언 데이터를 정제해 453k 병렬 페어를 구축하고, 표준어와 방언이 완전히 같은 샘플은 제거해 학습 신호 밀도를 높였어요.',
            'bilingual.feat2': '멀티태스크 학습: `<dialect_to_standard>`, `<standard_to_dialect>` 스페셜 토큰을 도입해 하나의 모델로 양방향 번역을 구현했어요.',
            'bilingual.feat3': '학습 파이프라인: Seq2SeqTrainer 기반으로 Full/LoRA/QLoRA 실험을 체계적으로 수행했어요.',
            'bilingual.feat4': '정량 평가: BLEU와 ROUGE-L을 기준으로 모델 품질을 비교 검증했어요.',
            'bilingual.feat5': '최종 성능: BLEU 0.56, ROUGE-L 0.60을 달성해 안정적인 번역 품질을 확인했어요.',
            'bilingual.chal1': 'OOM 이슈: Colab T4(16GB) 환경에서 Full Fine-tuning 중 Optimizer 상태 메모리로 OOM이 반복 발생했어요.',
            'bilingual.chal2': '메모리 최적화: QLoRA 4-bit 양자화를 적용해 메모리 사용량을 크게 줄이면서 성능 저하를 최소화했어요.',
            'bilingual.chal3': '성능 검증: 추가 리소스 환경에서 Full 모델과 QLoRA 모델을 비교해 성능-효율 트레이드오프를 분석했어요.',
            'bilingual.chal4': '검증 결과: QLoRA가 낮은 메모리 점유율에서도 Full Fine-tuning에 준하는 성능을 유지함을 확인했어요.',
            'bilingual.result1': '성과: 태그 기반 프롬프팅으로 단일 모델 양방향 번역 시스템을 안정적으로 구현했어요.',
            'bilingual.result2': '성과: 저사양 GPU 환경에서도 QLoRA 기반으로 실사용 가능한 수준의 번역 모델을 구축할 수 있음을 확인했어요.',
            'bilingual.result3': '인사이트: 번역 중심 과제에서는 인코더-디코더 아키텍처의 구조적 강점이 여전히 유효하다는 점을 다시 확인했어요.',
            'bilingual.result4': '향후 계획: 동일 파라미터 규모의 Decoder-only 모델과 정량 비교해 아키텍처 효율을 추가 분석할 예정이에요.',

            // tiny-DDPM
            'ddpm.overview': 'DDPM의 핵심 메커니즘을 처음부터 직접 구현해 생성 모델의 작동 원리를 코드 수준에서 검증하고, MNIST 생성 품질을 정량 지표로 최적화한 프로젝트예요.',
            'ddpm.problem': '문제: 최신 확산 모델의 수식과 아키텍처를 라이브러리 추상화에 가리지 않고 직접 구현하며 이해할 필요가 있었어요.',
            'ddpm.goal1': '목표 1: Ho et al. (2020) 논문의 수식을 코드로 정확히 옮기는 구현 역량을 확보해요.',
            'ddpm.goal2': '목표 2: DDIM, CFG를 추가 구현해 생성 속도와 품질을 동시에 개선해요.',
            'ddpm.goal3': '목표 3: FID/IS 기반 정량 평가 체계를 구축해 하이퍼파라미터 영향을 검증해요.',
            'ddpm.feat1': 'Noise Scheduling 구현: 선형/코사인 스케줄러와 Forward/Reverse Diffusion 로직을 직접 설계했어요.',
            'ddpm.feat2': 'Custom U-Net 설계: ResNet 기반 다운/업샘플링과 Residual Connection, Conditioning 강화를 위한 Attention 구조를 적용했어요.',
            'ddpm.feat3': '고급 샘플링: Non-Markovian DDIM과 Classifier-Free Guidance를 구현해 추론 효율을 높였어요.',
            'ddpm.feat4': '정량 평가: FID와 IS로 생성 품질을 수치화하고 Guidance Scale별 성능을 비교했어요.',
            'ddpm.feat5': '최적 지점 확인: CFG Scale w=0.1에서 FID 0.2514를 기록해 최고 성능을 확인했어요.',
            'ddpm.chal1': '조건 신호 간섭: Time Embedding과 Class Embedding을 단순 합산했을 때 특정 숫자가 생성되지 않는 Mode Collapse가 발생했어요.',
            'ddpm.chal2': '원인 분석: 시간 정보와 클래스 정보가 동일 채널로 주입되어 정보 간섭이 발생했어요.',
            'ddpm.chal3': '해결: Time/Class 경로를 분리하고 U-Net Bottleneck에 Cross-Attention 레이어를 도입해 조건 정보를 명확히 반영했어요.',
            'ddpm.chal4': '결과: 조건 정보 간섭이 해소되어 0~9 조건부 숫자를 안정적으로 생성하는 모델 학습에 성공했어요.',
            'ddpm.result1': '성과: DDPM, DDIM, CFG를 포함한 End-to-End 생성 파이프라인을 자체 구현했어요.',
            'ddpm.result2': '성과: FID 0.2514를 포함한 정량 지표로 성능을 검증하고, Guidance Scale의 영향을 실험적으로 확인했어요.',
            'ddpm.result3': '배운 점: 단순 재현을 넘어서 Attention 추가 같은 구조 개선까지 시도하며 아키텍처 엔지니어링 감각을 키웠어요.',
            'ddpm.result4': '배운 점: 실험 설정과 결과 기록의 중요성을 체감하며 체계적인 실험 관리 프로세스를 정립했어요.',

            // tiny-stable-diffusion
            'sd.overview': 'Stable Diffusion 3(SD3) 계열 아키텍처를 처음부터 구현해 64x64 텍스트-이미지 생성 모델을 구축한 프로젝트예요. U-Net이 아니라 Transformer 기반 MMDiT 구조를 직접 설계했어요.',
            'sd.problem': '문제: MNIST 기반 Diffusion 프로젝트의 한계를 넘어 Latent Diffusion과 Text Conditioning을 실제 코드로 구현하고 검증할 필요가 있었어요.',
            'sd.goal1': '목표 1: SD3 핵심 구조인 MMDiT + VAE 아키텍처를 자체 구현해요.',
            'sd.goal2': '목표 2: 텍스트 프롬프트 입력부터 64x64 RGB 이미지 생성까지 이어지는 end-to-end 학습/평가 파이프라인을 구축해요.',
            'sd.goal3': '목표 3: FID, CLIPScore 등 정량 지표 기반으로 모델 품질을 객관적으로 검증해요.',
            'sd.feat1': 'MMDiT 구현: Transformer 백본과 Joint Attention으로 이미지-텍스트 간 상호작용을 학습했어요.',
            'sd.feat2': 'Conditioning 설계: CLIP-L 텍스트 인코더와 adaLN-zero를 적용해 Time/Text 임베딩 주입 구조를 안정화했어요.',
            'sd.feat3': 'Rectified Flow + VAE: Rectified Flow와 beta-VAE를 적용해 잠재 공간 학습 효율을 높이고 VAE MSE 0.0002를 달성했어요.',
            'sd.feat4': '대규모 학습 인프라: LAION/CC3M 데이터 전략, Hugging Face Streaming, W&B + RunPod(A4500) 모니터링 체계를 구축했어요.',
            'sd.feat5': '정량 평가 체계: FID, CLIP-FID, IS, CLIPScore, PSNR, MSE를 포함한 다각도 평가 파이프라인을 구현했어요.',
            'sd.chal1': 'Prompt Overfitting: 소규모 데이터셋(30k)에서는 프롬프트와 무관한 패턴 반복 생성이 발생했어요.',
            'sd.chal2': '일반화 개선: 학습 데이터를 CC3M(3M)으로 확장하고 VAE:Diffusion 데이터 비율을 조정해 과적합을 완화했어요.',
            'sd.chal3': '수렴 지연: Diffusion 학습이 느리고 불안정해 장기 학습에서 손실 감소가 더뎠어요.',
            'sd.chal4': '학습 안정화: Logit-normal sampling, Min-SNR weighting, Checkpoint/Resume를 도입해 학습 안정성과 지속 가능성을 높였어요.',
            'sd.result1': '성과: MMDiT와 Rectified Flow를 포함한 SD3 핵심 요소를 직접 구현한 텍스트 조건부 생성 모델을 완성했어요.',
            'sd.result2': '성과: 눈대중 평가가 아니라 정량 지표로 모델을 검증하는 체계를 구축했어요.',
            'sd.result3': '성과: 64x64 해상도의 텍스트-이미지 생성 파이프라인을 end-to-end로 완성했어요.',
            'sd.result4': '배운 점: 최신 아키텍처 구현, 데이터 전략 설계, 제한된 자원 안에서의 학습 운영 역량을 함께 키울 수 있었어요.',

            // Drama Analysis Pipeline
            'drama.confidential': '사내 프로젝트 — 소스 코드 및 세부 산출물은 비공개예요. 이 페이지의 수치와 구조는 인턴 재직 당시의 개인 기록을 바탕으로 작성되었어요.',
            'drama.overview': '드라마 대본의 줄거리·캐릭터 매력도·흥행 요소를 정량적으로 분석해 제작 의사결정을 지원하는 Multi-Agent 파이프라인을 구축한 프로젝트예요. 4인 팀으로 진행했으며, alpha-beta-gamma 테스트 기간 중 실제 대본 검토 업무에 투입됐어요.',
            'drama.problem': '문제: 1~4화 분량 대본을 인력이 직접 검토하는 기존 프로세스는 시간·비용이 크고, 검토자 편향으로 평가 일관성이 낮았어요. 물리적 한계로 가치 있는 작품이 누락될 위험도 있었어요.',
            'drama.goal1': '목표 1: Multi-Agent 시스템으로 대본 분석을 자동화하고 객관적 평가 지표를 제공해요.',
            'drama.goal2': '목표 2: PDF/HWP/DOCX 등 다양한 포맷을 높은 한국어 정확도로 안정적으로 파싱해요.',
            'drama.goal3': '목표 3: OCR 품질(WER)과 씬 분류 성능(F1)을 실무 적용 가능한 수준으로 정량 개선해요.',
            'drama.feat1': 'Multi-Format Document Parser: PDF·HWP·DOCX를 정형 텍스트로 변환하는 전처리 모듈을 개발하고, 한국어 OCR 벤치마크 데이터셋으로 성능을 검증했어요.',
            'drama.feat2': 'VLM 도입 (Qwen2.5-7B-VL): 기존 OCR의 한국어 인식 한계와 다단 레이아웃 정렬 오류를 극복하기 위해 시각 구조를 이해하는 VLM을 적용했어요.',
            'drama.feat3': 'Scene Analysis Agent: 씬 단위 강점/약점 분류 에이전트에 CoT 프롬프팅을 결합해 한국어 구어체의 미묘한 뉘앙스까지 추론하도록 설계했어요.',
            'drama.feat4': 'AWS On-Demand GPU 배포: boto3로 필요한 시점에만 GPU EC2를 생성하고, 사전 초기화된 EBS volume mount·AMI·startup script를 조합해 cold start를 최소화하면서 유휴 GPU 비용을 제거했어요.',
            'drama.feat5': 'LangChain 오케스트레이션: Parser·Analyzer·Evaluator 에이전트를 상태 기반 워크플로우와 조건부 병렬 실행으로 연결했어요.',
            'drama.chal1': 'OCR 한계 — WER 20% 이상: PaddleOCR 등 기존 OCR은 한국어 인식률이 낮고, 다단 레이아웃에서 텍스트 순서가 뒤섞이는 정렬 오류가 빈번했어요.',
            'drama.chal2': '해결: 문서 시각 구조를 이해하는 Qwen2.5-7B-VL로 전환해 WER를 20%에서 7%로 대폭 낮췄어요.',
            'drama.chal3': '씬 분류 성능 — F1 0.2: 데이터 부족과 장르별 과도한 프롬프트 세분화로 모델이 표면 패턴에 과적합돼 F1이 0.2에 머물렀어요.',
            'drama.chal4': '해결: 한국어 추론 능력이 높은 모델(Deepseek-R1, c4ai-command-a 등)을 벤치마킹하고 프롬프트를 단순화해 모델 자체 추론에 맡김으로써 F1을 0.5로 개선했어요.',
            'drama.chal5': 'AWS Cold Start & 비용: S3에서 매번 weight를 받는 구조는 cold start가 20~30분에 달했어요. 사전 초기화된 EBS volume, AMI 스냅샷, uv 기반 환경 구성으로 기동 시간을 단축하고 리전별 GPU 가용성을 고려한 capacity-aware 리전 선택을 적용했어요.',
            'drama.result1': 'OCR 정확도: VLM 기반 파싱으로 WER 20% → 7% 달성, 하위 에이전트에 신뢰도 높은 입력 데이터를 확보했어요.',
            'drama.result2': '분류 성능: 모델 선정 및 프롬프트 단순화로 씬 단위 F1 스코어 0.2 → 0.5로 2.5배 향상됐어요.',
            'drama.result3': '실제 운영: alpha-beta-gamma 테스트 기간 실제 검토 업무에 투입. On-Demand GPU 구조로 주기성 워크로드에 맞는 비용 효율적 운영 구조를 확보했어요.',
            'drama.result4': '인사이트: 데이터 품질·단순한 프롬프트 설계·EBS mount 안정성·리전 GPU capacity 같은 인프라 세부 요소가 모델 성능만큼이나 실제 운영 가능성을 결정해요.',

            // tiny-chatbot-agent
            'chatbot.overview': '금융 서비스 FAQ와 이용약관(ToS)을 바탕으로 정확한 답변을 제공하는 금융 도메인 특화 RAG 에이전트를 개발했어요. FAQ 우선 검색과 약관 정밀 검색을 결합한 2단계 파이프라인으로 속도와 정확도를 함께 확보했어요.',
            'chatbot.problem': '문제: 사용자 용어와 약관 용어 간 의미 간극, 유사 조항 혼동, 환각 응답으로 인해 금융 도메인 챗봇의 신뢰성과 검색 정확도가 낮았어요.',
            'chatbot.goal1': '목표 1: FAQ 우선 검색 후 약관 심화 검색으로 이어지는 이중 단계 검색 구조를 구축해요.',
            'chatbot.goal2': '목표 2: 하이브리드 검색과 Cross-Encoder 재랭킹으로 조항 단위 정밀 검색 성능을 높여요.',
            'chatbot.goal3': '목표 3: 환각 검증과 자동 평가 체계를 도입해 답변 신뢰성을 정량 관리해요.',
            'chatbot.feat1': 'Dual-Stage RAG: Stage 1에서 FAQ를 빠르게 검색하고, 실패 시 Stage 2에서 ToS 전문을 정밀 탐색해요.',
            'chatbot.feat2': 'Hybrid Search + Reranking: 벡터/키워드/메타데이터 결합 검색과 Cross-Encoder 재정렬로 문맥 적합도를 개선했어요.',
            'chatbot.feat3': 'Hallucination Verifier: 생성된 답변이 검색 근거에 실제로 기반하는지 스스로 점검하는 검증 에이전트를 구현했어요.',
            'chatbot.feat4': 'LLM Judge 평가: Accuracy, Faithfulness, Completeness 지표와 적대적 시나리오를 포함한 자동 평가 파이프라인을 구축했어요.',
            'chatbot.feat5': 'MCP 확장성: Model Context Protocol을 적용해 에이전트가 필요할 때 도구를 호출할 수 있는 워크플로우를 구현했어요.',
            'chatbot.feat6': '로컬 추론 환경: vLLM/Ollama 기반으로 외부 전송 없이 동작 가능한 보안 친화형 구성을 완성했어요.',
            'chatbot.chal1': '용어 불일치: 사용자 질문의 표현(예: 환불)과 약관 표현(예: 청약 철회)이 달라 검색 누락이 발생했어요.',
            'chatbot.chal2': '문맥 혼동: 일반 해지와 직권 해지처럼 비슷한 조항이 섞여 잘못된 근거 문서가 선택되는 문제가 있었어요.',
            'chatbot.chal3': '해결: 벡터/키워드 점수를 가중 합산하는 Hybrid Search와 Cross-Encoder 재랭킹을 도입했어요.',
            'chatbot.chal4': '결과: 단순 벡터 검색 대비 Top-5 Recall을 약 20% 향상시키고 최종 답변 정확도를 개선했어요.',
            'chatbot.result1': '성과: 단순한 Retrieve-Generate를 넘어 Retrieve-Verify-Evaluate 전체 사이클을 구축했어요.',
            'chatbot.result2': '성과: Top-5 Recall 약 20% 개선과 함께 금융 도메인 질의 응답 신뢰도를 높였어요.',
            'chatbot.result3': '성과: MCP 기반 구조로 향후 도구 확장과 에이전트 오케스트레이션 유연성을 확보했어요.',
            'chatbot.result4': '배운 점: 평가셋과 자동 지표를 중심으로 품질을 관리하는 데이터 중심 개발 프로세스를 정착시켰어요.',

            // tiny-graph-rag
            'graphrag.overview': '비정형 문서에서 엔티티와 관계를 추출해 지식 그래프를 만들고, 그 관계 맥락을 반영해 문맥 검색을 수행하는 그래프 기반 RAG 시스템을 직접 구현한 프로젝트예요.',
            'graphrag.problem': '문제: 기존 벡터 검색은 의미 유사도 중심이라 인물·사건·개념 간 관계 구조를 반영한 추론에 한계가 있어요.',
            'graphrag.goal1': '목표 1: 외부 그래프 프레임워크에 크게 의존하지 않고 Graph RAG 핵심 로직을 직접 구현해요.',
            'graphrag.goal2': '목표 2: 문서로부터 지식 그래프를 자동 생성하고 질의별 최적 서브그래프를 탐색/랭킹해요.',
            'graphrag.goal3': '목표 3: 관계 기반 문맥 검색으로 멀티홉 질의 응답 품질을 개선해요.',
            'graphrag.feat1': '추출 엔진: LLM으로 노드와 엣지를 JSON 형태로 추출하는 파서를 개발했어요.',
            'graphrag.feat2': '데이터 모델링: Custom Graph Class와 JSON persistence를 설계해 그래프 저장·재사용 구조를 구현했어요.',
            'graphrag.feat3': '검색 및 랭킹: Seed Node 기반 BFS 탐색과 중심성/연관성 점수를 결합한 서브그래프 랭킹 로직을 구현했어요.',
            'graphrag.feat4': '시각화: Streamlit + PyVis 대시보드로 그래프 탐색과 RAG 응답 과정을 실시간으로 확인할 수 있게 했어요.',
            'graphrag.chal1': '호출 병목: 긴 문서를 청킹해 순차 호출할 때 그래프 구축 시간이 선형적으로 증가했어요.',
            'graphrag.chal2': '속도 개선: asyncio 기반 비동기 배치 처리로 엔티티 추출 파이프라인 처리량을 높였어요.',
            'graphrag.chal3': '동일 개체 중복: 김첨지/김씨/남편처럼 동일 인물이 중복 노드로 분리되어 연결성이 떨어졌어요.',
            'graphrag.chal4': 'Entity Resolution: 규칙 기반 + LLM 기반 하이브리드 ER, 유사도 0.75 임계치, 관계 충돌 검사를 통해 중복 노드를 50개에서 15개로 줄였어요.',
            'graphrag.result1': '성과: 전처리, 그래프 구축, 검색, 생성까지 Graph RAG 전 과정을 end-to-end로 직접 구현했어요.',
            'graphrag.result2': '성과: ER 고도화로 중복 노드를 약 70% 줄여 그래프 밀도와 검색 정확도를 개선했어요.',
            'graphrag.result3': '성과: 단순 텍스트 검색으로는 놓치기 쉬운 숨은 관계를 시각적으로 탐색하고 답변에 반영할 수 있었어요.',
            'graphrag.result4': '배운 점: Graph RAG 성능은 모델 자체보다 Entity Resolution 같은 전처리 품질에 크게 좌우된다는 점을 확인했어요.',

            // Medical Image Classification
            'medimg.overview': '맞춤형 CNN 아키텍처와 Vision Transformer(ViT)를 함께 탐색한 의료 이미지 분류 프로젝트예요. X-ray 분류를 중심으로 데이터 증강과 클래스 불균형 대응 기법을 적용했어요.',
            'medimg.problem': '문제: 의료 이미지 분류는 라벨이 부족하고 클래스가 불균형한 경우가 많으며, 진단에 활용하려면 높은 정확도도 요구돼요.',
            'medimg.goal1': '목표 1: 의료 이미지 분류에 대해 CNN과 ViT 아키텍처를 비교 평가해요.',
            'medimg.goal2': '목표 2: 제한된 라벨링 데이터를 보완할 수 있는 효과적인 데이터 증강 전략을 설계해요.',
            'medimg.goal3': '목표 3: 의료 데이터셋에서 흔한 클래스 불균형 문제를 완화해요.',
            'medimg.feat1': 'CNN 아키텍처: 의료 이미지에 맞춘 맞춤형 CNN을 설계하고 구현했어요.',
            'medimg.feat2': 'ViT 탐색: Vision Transformer를 적용하고 CNN과 성능을 비교했어요.',
            'medimg.feat3': '데이터 증강: 회전, 뒤집기, 밝기 조정 등 의료 이미지에 적합한 증강 기법을 구현했어요.',
            'medimg.feat4': '불균형 처리: 가중치 손실 함수와 오버샘플링으로 클래스 불균형을 완화했어요.',
            'medimg.chal1': '데이터 부족: 라벨링된 의료 이미지가 제한적이어서 데이터 증강과 전이 학습으로 이를 보완했어요.',
            'medimg.chal2': '클래스 불균형: 일부 질병 클래스가 매우 희소해 가중치 손실과 오버샘플링을 적용했어요.',
            'medimg.chal3': '모델 해석성: 의료 진단에서는 예측 근거가 중요하기 때문에 GradCAM으로 모델이 주목한 영역을 분석했어요.',
            'medimg.chal4': '일반화 성능: 학습 데이터와 다른 도메인에서 성능이 떨어지는 문제를 줄이기 위해 다양한 데이터 소스와 정규화 기법을 적용했어요.',
            'medimg.result1': '성과: CNN과 ViT 모두 높은 정확도를 기록했고, 일부 사례에서는 ViT가 더 좋은 성능을 보였어요.',
            'medimg.result2': '성과: 데이터 증강 기법이 제한된 데이터 환경에서 성능 향상에 크게 기여했어요.',
            'medimg.result3': '성과: GradCAM 시각화를 통해 모델이 적절한 병변 영역에 주목하고 있음을 확인했어요.',
            'medimg.result4': '배운 점: 의료 영상 분석, 데이터 불균형 대응, 모델 해석성 측면의 실전 경험을 쌓았어요.',

            // Synthetic OCR Image Generator
            'ocr.overview': '마크다운 중심 합성 OCR 데이터셋 생성 및 벤치마킹 툴킷이에요. LLM 기반 코퍼스 생성 → Headless Playwright 렌더링 → HuggingFace 퍼블리시 → 재현 가능한 OCR/VLM 모델 평가 → 리더보드까지 전 과정을 커버해요. 한국어와 일본어를 지원해요.',
            'ocr.problem': '문제: 한국어·일본어 OCR 고품질 라벨 데이터는 수집 비용이 높고, 텍스트·표·수식 영역을 동시에 공정하게 비교하는 재현 가능한 벤치마크가 존재하지 않아요.',
            'ocr.goal1': '목표 1: LLM 생성 코퍼스와 브라우저 렌더링을 활용해 한국어·일본어 OCR 합성 파이프라인을 구축해요.',
            'ocr.goal2': '목표 2: 체크포인트 재개, 모델 config YAML, 구조화된 리포트를 갖춘 재현 가능한 평가 프레임워크를 만들어요.',
            'ocr.goal3': '목표 3: 오픈 데이터셋을 HuggingFace Hub에 퍼블리시하고 리더보드를 지속적으로 운영해요.',
            'ocr.feat1': 'LLM 기반 코퍼스 생성: OpenAI API로 현실감 있는 한국어·일본어 텍스트 코퍼스를 생성하고, 생성·퍼블리시 워크플로우 전반에 재사용해요.',
            'ocr.feat2': 'Headless Playwright 렌더러: Chromium이 마크다운 페이지를 렌더링하며 노이즈·블러·문자 유사도 기반 오타 치환을 설정값으로 조절해요.',
            'ocr.feat3': '평가 파이프라인: YAML 모델 config, 배치 API, 체크포인트 재개를 지원하며 JSON/Markdown/HTML 리포트와 언어별 리더보드 파일을 출력해요.',
            'ocr.feat4': 'HuggingFace 퍼블리싱: 샤드 단위 업로드에 샤드별 메타데이터, realism_stats.json, run_manifest.json을 포함해 완전한 재현성을 보장해요.',
            'ocr.chal1': '수식 렌더링: 마크다운에 LaTeX 수식이 포함되어 XeLaTeX + latex-to-image 통합 및 장시간 실행을 위한 수식 렌더 캐싱이 필요했어요.',
            'ocr.chal2': '오타 현실감: 문자 유사도 데이터베이스를 구축해 실제 스캔 노이즈에 가까운 오타 치환을 구현했어요.',
            'ocr.chal3': '대규모 재개 처리: run_manifest.json 기반 샤드 출력으로 중간에 중단된 생성 작업을 중복 없이 이어받을 수 있도록 했어요.',
            'ocr.chal4': '평가 일관성: 체크포인트 기반 채점으로 중복 집계를 방지하고, 프로토콜 스냅샷으로 데이터셋 버전을 고정해 결과 비교 일관성을 유지했어요.',
            'ocr.result1': '한국어 리더보드: 1위 LightOnOCR-2-1B가 텍스트·표·수식 통합 avg_markdown_overall_score 0.9737 달성 (100/100 성공률).',
            'ocr.result2': '일본어 리더보드: 1위 모델 0.9777, Nanonets-OCR2-3B 0.9605, DotsOCR 0.9288로 모델 간 성능 차이를 정량 비교했어요.',
            'ocr.result3': '파이프라인 안정성: 생성·퍼블리시·평가 전 과정이 무중단으로 실행되어 반복적인 벤치마크 업데이트가 가능해요.',
            'ocr.result4': '배운 점: 합성 데이터 품질, 다국어 OCR 특성, 재현 가능한 ML 벤치마크 설계에 대한 깊은 이해를 쌓았어요.',

            // Application Projects
            // Pomodoro
            'pomodoro.overview': '포모도로 기법(25분 집중, 5분 휴식)을 바탕으로 만든 시간 관리 서비스예요. 구조화된 집중·휴식 사이클을 통해 생산성을 꾸준히 유지하도록 도와요.',
            ...
            // Delivery
            'delivery.overview': '배달 기사가 아파트 공동현관 비밀번호를 저장하고 공유할 수 있는 서비스예요. 출입 정보가 없어 생기는 배송 지연을 줄이는 것이 목표였어요. 처음에는 Android Native(Java)로 개발했고, 이후 Flutter로 옮겨 웹 데모까지 제공했어요.',
            ...
            // StudyWithMe
            'studywithme.overview': '소규모 학원을 위한 관리 애플리케이션이에요. 학생 정보, 출석, 강사 스케줄을 한곳에서 관리할 수 있도록 설계했어요.',
            ...
            // CJ AI Center Intern
            'cj.overview': 'CJ ENM AI 센터에서 6개월간 인턴으로 근무하며, 한국 드라마 대본을 분석하는 멀티에이전트 파이프라인 개발을 담당했어요.',
            ...
            'embedded.overview': 'UNIST Embedded AI Lab에서 연구 인턴으로 활동하며 DDPM 논문을 리뷰하고, MNIST 기반 토이 확산 모델을 학습하며, 샘플링 최적화 방향을 탐색했어요.',
            ...
            // Troja
            'troja.overview': '딥페이크 방어 기술을 개발하는 초기 스타트업 Troja에서 NAVER Cloud Platform 위에 딥페이크 생성 모델과 방어 모델을 함께 서빙하는 백엔드 서버를 구축했어요. inswapper 기반 생성과 LEAT·DIPA 방어 기법을 하나의 프로덕션 환경에서 테스트할 수 있게 했어요.',

            // Legacy Projects
            // Diffusion Model (Legacy)
            'difflegacy.overview': 'tiny-DDPM과 tiny-stable-diffusion으로 이어지기 전 단계의 초기 버전이에요. DDPM과 확산 모델을 이해하기 위해 만든 첫 구현이었어요.',
            ...
            'minigraphrag.overview': 'tiny-graph-rag 이전 단계의 프로젝트로, 지식 그래프와 벡터 검색을 결합한 RAG 시스템의 초기 프로토타입이에요.',
            ...
            // Gomoku MCP
            'gomoku.overview': 'Model Context Protocol(MCP)을 통해 LLM이 오목을 둘 수 있는 게임 서버예요. 위협 분석 엔진과 우선순위 기반 착수 추천을 MCP 툴로 노출해, 어떤 LLM이든 보드를 추론하고 전략적 수를 둘 수 있으며, AI vs AI 모드로 두 모델의 대결을 관전할 수 있어요.',
            'gomoku.feat1': '위협 분석 엔진 - Open-4, Closed-4, Open-3, 갭 패턴을 감지하고 우선순위(1~10)를 매겨 LLM의 착수 결정을 지원해요',
            'gomoku.feat2': 'MCP 툴 스위트 - set_stone, analyze_threats, get_suggested_moves 등 게임 전체 제어를 MCP 툴로 제공해요',
            'gomoku.feat3': 'AI vs AI 모드 - OpenRouter를 통해 Gemini, Grok, Kimi 등 서로 다른 LLM끼리 대결을 관전할 수 있어요',
            'gomoku.feat4': 'CLI & 웹 GUI - 커맨드라인 인터페이스와 브라우저 기반 GUI를 모두 지원해요',

            // tiny-agent-harness
            'harness.overview': 'OpenAI Codex CLI와 Anthropic Claude Code에서 보이는 supervisor 중심 멀티에이전트 하니스 구조를 작은 크기로 재구성한 토이 프로젝트예요. 계획, 실행, 리뷰, 툴 호출, 이벤트 채널을 모두 명시적으로 드러내어 읽고 실험하기 쉬운 형태로 만들었어요.',
            'harness.problem': '문제: 에이전트형 코딩 도구는 내부 오케스트레이션이 프레임워크 뒤에 숨어 있는 경우가 많아, 실제로 어떤 루프와 경계가 동작하는지 이해하고 수정하기 어려워요.',
            'harness.goal1': '목표 1: supervisor가 planner, worker, reviewer를 상황에 따라 반복 호출하는 하니스 구조를 작은 코드베이스로 재현해요.',
            'harness.goal2': '목표 2: 역할별 출력 스키마, 툴 권한, 이벤트 흐름을 분리해 각 레이어를 눈으로 추적 가능하게 만들어요.',
            'harness.goal3': '목표 3: CLI 실행과 라이브러리 임베딩 둘 다 가능한 형태로 구성해 실험과 확장을 쉽게 만들어요.',
            'harness.feat1': 'Supervisor-led 루프: 고정된 planner→worker→reviewer 체인이 아니라, supervisor가 다음 서브에이전트를 동적으로 선택하고 같은 역할을 여러 번 재호출할 수 있어요.',
            'harness.feat2': '명시적 역할 분리: planner는 읽기 중심 분석, worker는 수정/실행, reviewer는 검증에 집중하도록 입력·출력 스키마와 권한을 분리했어요.',
            'harness.feat3': 'YAML 기반 설정: provider, 역할별 모델, 툴 목록, retry 제한을 설정 파일로 바꿀 수 있어 구조 실험이 쉬워요.',
            'harness.feat4': '채널/이벤트 아키텍처: `run_started`, `tool_call_finished`, `run_result` 같은 이벤트를 listener/output 채널로 흘려 CLI 출력과 외부 관찰 로직을 분리했어요.',
            'harness.feat5': '배포 가능한 패키지: `tiny-agent` CLI 엔트리포인트를 제공하고, GitHub에서 바로 설치해 로컬 워크스페이스 대상 에이전트 실행이 가능해요.',
            'harness.result1': '성과: 실제 코딩 에이전트 제품군의 핵심 구조를 작은 코드베이스로 축약해, 아키텍처를 읽고 실험하기 좋은 학습용 하니스를 만들었어요.',
            'harness.result2': '성과: 대화형 CLI와 프로그램적 사용 방식을 모두 지원해 데모와 임베딩 실험을 같은 코드로 처리할 수 있어요.',
            'harness.result3': '성과: supervisor, planner, worker, reviewer, CLI에 대한 pytest 테스트를 갖춰 핵심 루프의 회귀를 점검할 수 있어요.',
            'harness.result4': '배운 점: 에이전트 신뢰성은 모델 자체보다도 구조화된 출력, 권한 경계, 이벤트 가시성 같은 오케스트레이션 설계에 크게 좌우된다는 점을 확인했어요.',

            // LLM Mafia
            'llmmafia.overview': '여러 LLM이 서로 다른 역할을 맡아 마피아 게임을 플레이하는 멀티에이전트 시뮬레이터예요. SpeechQueue 기반 토론 구조 덕분에 단순 턴제 응답이 아니라 실제 토론처럼 발언 요청과 반응이 오가는 상호작용을 관찰할 수 있어요.',
            'llmmafia.feat1': '멀티 에이전트 시뮬레이션 - 여러 모델이 시민, 마피아, 경찰, 의사 역할로 상호작용해요.',
            'llmmafia.feat2': 'SpeechQueue 토론 엔진 - 발언 요청 큐를 통해 더 자연스러운 토론 흐름과 후속 반응을 만들어요.',
            'llmmafia.feat3': '유연한 설정 - 역할 수, 플레이어 수, 모델 조합, random seed를 조정해 실험을 재현할 수 있어요.',
            'llmmafia.feat4': '시각화 대시보드 - Streamlit UI에서 게임 진행, 로그, 메트릭을 실시간으로 확인할 수 있어요.'
        }
    };
    ...
    var defaults = {
        ...
        'proj.features': 'Key Features & Contributions',
